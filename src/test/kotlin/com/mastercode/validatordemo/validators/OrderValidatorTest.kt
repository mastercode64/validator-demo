...
class OrderValidatorTest{
    ...
    @Test
    fun `order validator should inject correct rules`() {
        assertEquals(1, validator.rules.size)
        assertTrue(validator.rules.any { it is RuleInvalidDescription })
    }

    @Test
    fun `validator should validate only occurrence type ORDER`() {
        assertTrue(validator.shouldValidate(OccurrenceType.ORDER))
        assertFalse(validator.shouldValidate(OccurrenceType.TRANSACTION))
